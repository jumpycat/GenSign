...
def resize_up_only(image, min_size, interpolation=cv2.INTER_LINEAR, **kwargs):
...
def main(params):
    ...
    # --- Data Transforms (Albumentations Integration) ---
    albu_transform = A.Compose([
        A.Lambda(
            name="ResizeUpOnly",
            image=lambda img, **kwargs: resize_up_only(img, min_size=params.img_size, interpolation=cv2.INTER_LINEAR),
            p=1.0
        ),        
        ...
        A.ImageCompression(quality_lower=20, quality_upper=80, p=0.3),
        A.GaussNoise(p=0.14),
        A.MotionBlur(p=0.14),
        A.CLAHE(p=0.14),
        A.RandomGamma(p=0.2),
    ...
    train_transform = transforms.Compose([
        AlbumentationsTransform(transform=albu_transform),
        normalize_vqgan
    ])
    ...
    print("\n=== Starting Training (clip_dual Logic / ProGAN Data with Albumentations) ===")
